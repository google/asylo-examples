...
std::string GetPath(const std::string &flag) {
  ...
  free(path);
  return runfiles->Rlocation("asylo_examples/redis/" + base_name);
}
...
class RedisServerTest : public ::testing::Test {
 ...
 protected:
  ...
  void CheckExitStatus(int exit_status) {
    ...
    EXPECT_EQ(WEXITSTATUS(exit_status), 0) << absl::StrCat(
        "Subprocess exited with non-zero status ", WEXITSTATUS(exit_status));
  }
